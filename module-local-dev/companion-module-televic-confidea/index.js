...
instance.prototype.init = function() {
	...
	self.init_presets();
	//self.setFeedbackDefinitions(feedbacks);  // commented out while working on udp connector
	self.init_feedbacks()
...
instance.prototype.udp = function() {
	var self = this;
	var controller = self.udp.createSocket('udp4');
	var message = '';
	...
	self.controller.on('listening', function () {
		// log message
		console.log('Listening for UDP on ' + self.prefix + ":" + self.port);
	});
	
	// Messsage received
	if (self.udp.message !== undefined) {
		self.controller.on('message', function () {
			var udpmessage;

			// log message
			udpmessage = JSON.parse(message)
			console.log('Message from ' + self.prefix + ':' + self.port +' - ' + message);
		});
		
		//
		// Sort message for mic state
		switch (self.udpmessage(status)) {
